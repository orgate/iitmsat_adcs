...
%   function [time, rx, ry, rz] =testmat(yr, month, tday, hour, min, second)

 % function [time, rx, ry, rz] =testmat()
  %   yr, month, tday, hour, min, second
...
%   typerun = input('input type of run c, v, m: ','s');
    ...
    row=10;
    col=45;
    rx=10000*ones(row,col);
    ry=10000*ones(row,col);
    rz=10000*ones(row,col);
    vx=10000*ones(row,col);
    vy=10000*ones(row,col);
    vz=10000*ones(row,col);
    k=0;
    posi=[];
    %satrec1 stores the values used for initialization/ defining the start
    %time
    satrec1.year=yr;
    satrec1.mon=month;
    satrec1.day=tday;
...
%                // includes initialization of sgp4
            ...
            [satrec, startmfe, stopmfe, deltamin] = twoline2rv(satrec1, whichconst, ...
            ...
            while ((tsince < stopmfe) && (satrec.error == 0))
                ...
                if (satrec.error == 0)
                    if ((typerun ~= 'v') && (typerun ~= 'c'))
                        ...
                        rx(1+floor(k/col),1+rem(k,col))=ro(1);
                        ry(1+floor(k/col),1+rem(k,col))=ro(2);
                        rz(1+floor(k/col),1+rem(k,col))=ro(3);
                        vx(1+floor(k/col),1+rem(k,col))=vo(1);
                        vy(1+floor(k/col),1+rem(k,col))=vo(2);
                        vz(1+floor(k/col),1+rem(k,col))=vo(3);
                        time(1,k+1)=tsince;
                        k=k+1;
                        k
...
%     vz=vz(vz~=10000);
    ...
    posi=[rx; ry; rz];
    rx2=(rx-repmat(rx(row,:),row,1)).*(rx-repmat(rx(row,:),row,1));
    ry2=(ry-repmat(ry(row,:),row,1)).*(ry-repmat(ry(row,:),row,1));
    rz2=(rz-repmat(rz(row,:),row,1)).*(rz-repmat(rz(row,:),row,1));
    delerror=sqrt(rx2+ry2+rz2);
    plot(delerror)
