...
omega=10; %argument of perigee. Has no significance for a circular orbit. 
...
for oo=1:1 %varying inclination
    ...
    long=zeros(1,num);
    r_ECEF=zeros(3,num);
    [time r_ECEF lat long]=ECIOrbitModel(num, time_step);
    field=[];
    alti=sqrt((r_ECEF(1,:).*r_ECEF(1,:))+(r_ECEF(2,:).*r_ECEF(2,:))+(r_ECEF(3,:).*r_ECEF(3,:)))-6371.2;
   tic 
    global gh
...
%         
        ...
        B_ECEF_sph=igrf11syn(2011,alti(1,t),lat(t),long(t));
...
%         TA(:,:,t)=TargetAttitude([B_ECEF_sph(1);B_ECEF_sph(2);-B_ECEF_sph(3)]);
    ...
    putx=1:num;
    genx=1:.2:num;
    geny=spline(putx,B_field(:,1),genx);
    plot(genx,geny)
toc 
end
